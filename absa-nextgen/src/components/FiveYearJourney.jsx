...
// Track-specific : milestone labels per year + growth model
const trackConfig = {
  balanced: {
    yearLabels: [
      "Emergency Fund",
...
// Five Year Journey component: visualizes user's financial trajectory over 5 years based on current savings, monthly contributions, and track type. Shows milestones, progress, and gap to 5-year target with interactive adjustments.
export default function FiveYearJourney({
  trackKey,
  monthlyAmount,
  currentSaved = 0,
  fiveYearTarget = 0,
  ...
  const yearValues = [1, 2, 3, 4, 5].map((y) =>
    cfg.compute(monthly, currentSaved, y, monthsElapsed),
  );
  const year5Value = yearValues[4];
  const currentYearIndex = Math.min(
    4,
    Math.max(0, Math.floor(monthsElapsed / 12)),
  );

  // For catch-up track, lower is better (debt going to 0). change on-track logic.
  ...
  const clearYear = isCatchup
    ? yearValues.findIndex((v) => v === 0) + 1 || null
    : null;
  const debtCleared = isCatchup && year5Value === 0;

  const targetGap =
    !isCatchup && fiveYearTarget > 0
      ? Math.max(0, fiveYearTarget - year5Value)
      : 0;
  const onTrack =
    !isCatchup && fiveYearTarget > 0 && year5Value >= fiveYearTarget;

  // Progress bar: for catchup = % debt cleared so far (currentSaved = totalDebt, actual paid = 0 at start)
  const barPct = isCatchup
    ? totalDebt > 0
      ? Math.min(100, Math.round(((totalDebt - year5Value) / totalDebt) * 100))
      : 0
    : year5Value > 0
      ? Math.min(100, Math.round((currentSaved / year5Value) * 100))
      : 0;
    ...
    fiveYearTarget > 0
      ? trackKey === "balanced"
        ? (() => {
            ...
            return Math.round(
              ((fiveYearTarget - currentSaved) * r) / (Math.pow(1 + r, n) - 1),
            );
          })()
        : Math.round(
            (fiveYearTarget - currentSaved) / remainingProjectionMonths,
          )
      : 0;
      ...
      <div className="fy-wrap">
        ...
        <div className="fy-track">
          ...
          {[1, 2, 3, 4, 5].map((y, i) => {
            ...
            const isFuture = isCatchup
              ? yearValues[i] > 0
              : currentSaved < yearValues[i];
            const nodeCleared = isCatchup && yearValues[i] === 0;
            const isCurrentNode = i === currentYearIndex;
            return (
              <div
                key={y}
                className={`fy-node ${!isFuture ? "fy-node--done" : ""} ${y === 5 ? "fy-node--end" : ""} ${isCurrentNode ? "fy-node--current" : ""}`}
              >
                <div className="fy-dot">{!isFuture ? "✓" : y}</div>
                {isCurrentNode && (
                  <span className="fy-you-are-here">▲ You are here</span>
                )}
                ...
                {isCatchup ? (
                  <p
                    className="fy-value"
                    style={{ color: nodeCleared ? "#2a9d8f" : undefined }}
                  >
                    {nodeCleared
                      ? "Gone ✓"
                      : `R${yearValues[i].toLocaleString("en-ZA")} left`}
                  </p>
                ) : (
                  <p className="fy-value">
                    R{yearValues[i].toLocaleString("en-ZA")}
                  </p>
                )}
              </div>
            );
        ...
        <div className="fy-outcome">
          <div className="fy-outcome-left">
            <p className="fy-outcome-eyebrow">
              {isCatchup
                ? clearYear
                  ? `Debt Free by Year ${clearYear}`
                  : "By Year 5"
                : "By Year 5"}
            </p>
            {isCatchup ? (
              debtCleared ? (
                <p className="fy-outcome-number" style={{ color: "#2a9d8f" }}>
                  Debt Free ✓
                </p>
              ) : (
              ...
              <p className="fy-outcome-number">
                <NumberCounter value={year5Value} prefix="R" />
              </p>
            )}
            {isCatchup ? (
            ...
            {isCatchup ? (
              ...
              debtCleared ? (
                ...
                <div className="fy-badge fy-badge--gap">
                  <span>
                    Still R{year5Value.toLocaleString("en-ZA")} remaining after
                    5 years
                  </span>
                  <p className="fy-badge-hint">
                    Increase monthly payment to clear faster · go to Setup to
                    adjust
                  </p>
                </div>
              ) : (
                <button
                  className="fy-badge fy-badge--neutral fy-badge--link"
              ...
              onTrack ? (
                <div className="fy-badge fy-badge--good">
                  On track for your R{fiveYearTarget.toLocaleString("en-ZA")}{" "}
                  goal ✓
                </div>
              ) : (
                <div
                ...
                >
                  ...
                  >
                    <span>
                      R{targetGap.toLocaleString("en-ZA")} short of your goal
                    </span>
                    <p className="fy-badge-hint">
                      To hit R{fiveYearTarget.toLocaleString("en-ZA")}, invest R
                      {requiredMonthly.toLocaleString("en-ZA")}/month · tap to
                      adjust
                    </p>
                  </button>
                  ...
                  {showAdjust && (
                    <div className="fy-adjust-panel">
                      <p className="fy-adjust-title" style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        <Zap size={13} /> Readjust your 5-year goal
                      </p>
                      <p className="fy-adjust-hint">
                        At R{monthly.toLocaleString("en-ZA")}/month you'll reach{" "}
                        <strong>R{year5Value.toLocaleString("en-ZA")}</strong>{" "}
                        by Year 5. Update your target to match, or go to Setup
                        to increase your monthly contribution.
                      </p>
                      <div className="fy-adjust-row">
                        <div className="fy-adjust-option">
                          <p className="fy-adjust-option-label">
                            Match target to projection
                          </p>
                          <button
                            className="fy-adjust-btn fy-adjust-btn--primary"
                            onClick={() => {
                              updateUser({ fiveYearGoal: year5Value });
                              setShowAdjust(false);
                            }}
                          >
                        ...
                        <div className="fy-adjust-option">
                          ...
                          >
                            Go to Setup → invest R
                            {requiredMonthly.toLocaleString("en-ZA")}/month
                          </button>
                        </div>
                      </div>
                      <div className="fy-adjust-custom">
                        ...
                        <div className="fy-adjust-input-row">
                          ...
                          />
                          <button
                            className="fy-adjust-btn fy-adjust-btn--ghost"
