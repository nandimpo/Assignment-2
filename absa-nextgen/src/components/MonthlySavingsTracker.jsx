...
export default function MonthlySavingsTracker({
  ...
  const progress =
    goalAmount > 0
      ? Math.min(100, Math.round((totalSaved / goalAmount) * 100))
      : 0;
  const remaining = Math.max(0, goalAmount - totalSaved);
  const monthsLeft =
    monthlyTarget > 0 ? Math.ceil(remaining / monthlyTarget) : "—";
  ...
  const confirmSaved = (amount = monthlyTarget) => {
    ...
    const newLog = [
      ...log,
      { month: thisMonth, amount: Number(amount), missed: false },
    ];
    updateUser({
      [logKey]: newLog,
      savings:
        logKey === "savingsLog" ? totalSaved + Number(amount) : undefined,
    });
  ...
  const removeEntry = (month) => {
    ...
    const newSavings = newLog
      .filter((e) => !e.missed)
      .reduce((sum, e) => sum + e.amount, 0);
    updateUser({
      [logKey]: newLog,
      ...(logKey === "savingsLog" ? { savings: newSavings } : {}),
    });
  ...
  const missedTips = [
    {
      icon: <Lightbulb size={14} color="#d6a85a" />,
      tip: "Review your fixed expenses — even a R500 cut frees up meaningful savings.",
    },
    {
      icon: <RefreshCw size={14} color="#84a794" />,
      tip: `Try saving R${catchUpAmount.toLocaleString("en-ZA")} next month to catch up (50% extra).`,
    },
    {
      icon: <CalendarClock size={14} color="#84a794" />,
      tip: "Set up an automatic transfer on payday — save before you spend.",
    },
    {
      icon: <Target size={14} color="#d6a85a" />,
      tip: `Your goal is R${goalAmount.toLocaleString("en-ZA")}. Every month missed adds ~1 month to your timeline.`,
    },
    {
      icon: <BarChart2 size={14} color="#8fa3a0" />,
      tip: "Check your spending breakdown in the Snapshot — lifestyle and transport are usually the biggest levers.",
    },
  ];
      ...
      <div className={`mst-card ${className}`.trim()}>
        ...
        <div className="mst-header">
          <div>
            ...
            <p className="mst-sub">
              Log R{monthlyTarget.toLocaleString("en-ZA")}/month toward your{" "}
              {goalLabel}
            </p>
          </div>
          <div className="mst-totals">
            <div className="mst-stat money-glow">
              <span className="mst-stat-label">
                {verb === "paid" ? "Paid total" : "Saved total"}
              </span>
              <strong className="mst-stat-value accent">
                <NumberCounter value={totalSaved} prefix="R" />
              </strong>
            </div>
            <div className="mst-stat">
              <span className="mst-stat-label">Months left</span>
              <strong className="mst-stat-value">
                {Number.isFinite(Number(monthsLeft))
                  ? Number(monthsLeft).toLocaleString("en-ZA")
                  : monthsLeft}
              </strong>
            </div>
            <div className="mst-stat">
              ...
              <strong className="mst-stat-value">
                <NumberCounter value={progress} suffix="%" />
              </strong>
            </div>
          </div>
        </div>
        ...
        <div className="mst-progress-bar">
          <div
            className="mst-progress-fill"
            style={{ width: `${progress}%` }}
          />
        </div>
        <div className="mst-streak">
          <span className="mst-streak-item saved">
            <CheckCircle size={11} /> {savedMonths} {verb}
          </span>
          {missedMonths > 0 && (
            <span className="mst-streak-item missed">
        ...
        {!alreadyLogged ? (
          <div className="mst-checkin">
            ...
            <div style={{ flex: 1 }}>
              <p className="mst-checkin-title">
                {formatMonth(thisMonth)} — Did you{" "}
                {verb === "paid" ? "pay" : "save"} R
                {monthlyTarget.toLocaleString("en-ZA")}?
              </p>
              <p className="mst-checkin-sub">
                Be honest — tracking missed months helps you recover faster
              </p>
            </div>
            <div className="mst-checkin-btns">
              <button className="mst-yes-btn" onClick={() => confirmSaved()}>
                <CheckCircle size={13} /> Yes
              </button>
              <button className="mst-no-btn" onClick={confirmMissed}>
                <XCircle size={13} /> No
              </button>
            </div>
          </div>
          ...
          <div className="mst-checkin mst-checkin--missed">
            ...
            <div style={{ flex: 1 }}>
              <p className="mst-checkin-title">
                Missed {formatMonth(thisMonth)}
              </p>
              <p className="mst-checkin-sub">See recovery tips below</p>
            </div>
            <button
              className="mst-remove-inline"
              onClick={() => removeEntry(thisMonth)}
            >
              Undo
            </button>
          </div>
          ...
          <div className="mst-checkin mst-checkin--done">
            ...
            <div style={{ flex: 1 }}>
              <p className="mst-checkin-title">
                {verb === "paid" ? "Paid" : "Saved"} in {formatMonth(thisMonth)}
              </p>
              <p className="mst-checkin-sub">
                R{thisEntry?.amount.toLocaleString("en-ZA")} logged — great
                work!
              </p>
            </div>
            <button
        ...
        {(showMissedPanel || (alreadyLogged && thisEntry?.missed)) && (
          <div className="mst-missed-panel">
            ...
            <div className="mst-catchup">
              <RefreshCw size={14} />
              <p>
                Did you manage to save <em>something</em> this month?
              </p>
            </div>
            ...
            <div className="mst-adjust-suggestion">
              ...
              <p className="mst-adjust-text">
                ...
                <strong className="accent">
                  R{catchUpAmount.toLocaleString("en-ZA")}
                </strong>{" "}
                next month to make up for this miss. That's your normal R
                {monthlyTarget.toLocaleString("en-ZA")} + 50% extra.
              </p>
              <p className="small" style={{ marginTop: 4, color: "#8fa3a0" }}>
                Your new estimated timeline:{" "}
                <strong>
                  {monthlyTarget > 0
                ...
                (unchanged if you catch up next month)
              </p>
            </div>
          </div>
        )}
          ...
          <div className="mst-history">
            ...
            <div className="mst-history-grid">
              {[...log].reverse().map((entry) => (
                <div
                  key={entry.month}
                  className={`mst-history-item ${entry.missed ? "mst-history-item--missed" : ""}`}
                >
                  {entry.missed ? (
                  ...
                  <span
                    className={entry.missed ? "mst-missed-label" : "accent"}
                  >
                    {entry.missed
                      ? "Missed"
                      : `+R${entry.amount.toLocaleString("en-ZA")}`}
                  </span>
                  <button
